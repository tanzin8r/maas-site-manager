...
import { render, screen, setupServer, userEvent, waitFor } from "@/test-utils";
...
describe("UserForm", () => {
  ...
  it("prefills data for a user when editing", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("textbox", { name: "Username" })).toHaveValue(user.username));
