...
`msm.apiserver.create_app()`. This adds middlewares and mounts the sub-applications
...
e.g. by `authenticated_user` or `authenticated_admin` for the `msm.apiserver.user`
sub-application, or `authenticated.site` for the `msm.apiserver.site` one). The
...
The `msm.apiserver.db` module contains tables definitions (under `msm.apiserver.db.tables`) as well
custom DB types and generic helpers to generate queries (e.g. related to
counting and filtering).

The `msm.apiserver.db.models` submodule also contains some Pydantic models which are
...
Finally, the `msm.apiserver.db.alembic` package contains the database migrations setup,
...
- `msm.apiserver.schema` contains helpers and schema definitions related to common API
...
The bulk of the logic is implemented in `msm.common.jwt`, which defines a `JWT`
...
Validation and authentication logic can be found in `msm.apiserver._auth`, as well as
`msm.apiserver.<api-name>._auth` for service-specific bits.
